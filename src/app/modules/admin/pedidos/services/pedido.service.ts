...
export class PedidoService {
  private _pedidoActual: BehaviorSubject<ProductsAdded[]> = new BehaviorSubject<
    ProductsAdded[]
  >(null);
  ...
  // constructor
  constructor(private _http: HttpClient, private _notify: NotifierService) {
  ...
  createOrder(
    ...
    return this._http
      .post<CreateOrder>(`${this.endPointCreateOrder}`, body)
      .pipe(
        tap((_) => {
  ...
  public async addProductToCurrentOrder(currentProducto: ProductsAdded) {
    ...
    try {
      ...
      let buscarProductoById = findIndex(currentOrder, {
      ...
      // si ya se encuentra emitimos una alerta
        ...
        this._notify.showNotification(
          'Este producto esta agregado',
          'Elimenelo para volver agregar',
          'warning',
          null
        );
    }

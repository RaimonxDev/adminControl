...
import { HandleHttpResponseError } from '../../../../shared/utils/class/httpErrorHandler';
...
export class CustomerService {
  ...
  constructor(
    ...
    this._userServices.SellerID$.subscribe(
      (user) => (this._currentSellerId = user)
    );
  }
  ...
  getUsers(): Observable<User[]> {
    return this._http
      .get<User[]>(`${this.urlBackend}/users`, {
        params: {
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          return HandleHttpResponseError(
            error,
            'Sin Conexcion a la base de datos'
          );
        })
      );
  }
  ...
  private UpdataDataOfNewUser(idNewAccount: string, data: User) {
    ...
    return this._http
      .put<User>(`${this.urlBackend}/users/${idNewAccount}`, body)
      .pipe(
        tap((_) =>
          this._notifier.showNotification(
  ...
  public registerNewCustomerAccount(data: User): Observable<any> {
    ...
    return this._http
      .post<UserSignIn>(`${this.urlBackend}/auth/local/register`, body)
      .pipe(
        tap((_) =>
          this._notifier.showNotification(
            'Espere Por Favor',
        ...
        catchError((err) =>
          HandleHttpResponseError(err, err.error.message[0].messages[0].message)
        )
      );
  }

  public updateData(body: User, id: string) {
    return this._http.put(`${this.urlBackend}/users/${id}`, body).pipe(
      tap((customerUpdate: User) => this._user.next(customerUpdate)),
      switchMap((_) => {
        return this.getUsers();
      }),
      catchError((err) =>
        HandleHttpResponseError(err, 'Error al actulizar data')
      )
    );
  }
  ...
  //----------------------- UTILS----------------------------------------------
  getRegions() {
    return this._http.get<Regions[]>(this.endPointRegions).pipe(
      tap((regions) => {
